...
#   systemd/disposable preflight тЖТ lab MSSQL+CP тЖТ seed location/bootstrap тЖТ
#   install published server-vFROM with --test-self-signed тЖТ legacy ACME fixture тЖТ
#   Playwright node-update button тЖТ wait for TARGET тЖТ assert PID change + durable SUCCESS тЖТ
#   (when Pebble enabled) install locally built 1.1.18 candidate тЖТ ACME/cert/TLS/QUIC/rollback.
#
...
leaf_fingerprint_sha256() {
  # stdin: PEM certificate тЖТ lowercase hex SHA-256 of DER
...
if [[ "${DURABLE_RESTART_RESULT}" == "PASS" ]] && { [[ "${ENABLE_PEBBLE}" == "1" ]] || [[ -n "${PEBBLE_DIR_URL}" ]]; }; then
  ...
  # Ensure Pebble is reachable on host network for HTTP-01 тЖТ :80
  ...
  # 1.1.17 update should already have migrated legacy root:root тЖТ nyxveil:nyxveil 0700.
  ...
  # Allow ACME issuance on first start (self-signed тЖТ Pebble leaf)
