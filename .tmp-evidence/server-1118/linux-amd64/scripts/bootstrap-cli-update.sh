#!/usr/bin/env bash
# bootstrap-cli-update.sh РІР‚вЂќ safe CLI-first path for legacy nyxveilctl updaters
#
...
# Fail-closed: bad hash / arch / version РІвЂ вЂ™ old CLI remains intact.
#
# Trust model:
...
#
# Usage (production):
#   sudo bash bootstrap-cli-update.sh --version 1.1.18 --then-update
#
# Offline:
...
VERSION="${NYXVEIL_BOOTSTRAP_VERSION:-1.1.18}"
...
Usage: bootstrap-cli-update.sh [options]
  ...
  --version X.Y.Z     Target release version (default 1.1.18)
