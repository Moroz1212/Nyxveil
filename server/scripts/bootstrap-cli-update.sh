...
# Fail-closed: bad hash / arch / version → old CLI remains intact.
#
# Trust model:
...
#
# Usage (production):
#   sudo bash bootstrap-cli-update.sh --version 1.1.10 --then-update
#
# Offline:
...
VERSION="${NYXVEIL_BOOTSTRAP_VERSION:-1.1.10}"
...
Usage: bootstrap-cli-update.sh [options]
  ...
  --version X.Y.Z     Target release version (default 1.1.10)
