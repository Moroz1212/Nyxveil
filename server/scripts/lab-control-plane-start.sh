...
# Write sourcable env with shell-safe quoting (CS contains ";User Id=...").
umask 077
export CP_BASE="https://127.0.0.1:${CP_PORT}"
export CP_PORT
export CP_CA_PEM="${CA_PEM}"
export CP_ADMIN_USER="${ADMIN_USER}"
export CP_ADMIN_PASSWORD="${ADMIN_PASSWORD}"
export CP_LOCATION_ID="${LOCATION_ID}"
export CP_BOOTSTRAP_TOKEN="${BOOTSTRAP_RAW}"
export CP_LICENSE_KEK_HEX="${LICENSE_KEK_HEX}"
export CP_SQL_CS="${CS}"
export CP_MSSQL_CONTAINER="${MSSQL_CONTAINER}"
export CP_DB_NAME="${DB_NAME}"
export CP_SA_PASSWORD="${SA_PASSWORD}"
export CP_PID_FILE="${PID_FILE}"
export CP_WORK_DIR="${WORK_DIR}"
export CP_WEB_DLL="${WEB_DLL}"

python3 - "${OUTPUT_ENV}" <<'PY'
import os, shlex, sys
path = sys.argv[1]
keys = [
    "CP_BASE", "CP_PORT", "CP_CA_PEM", "CP_ADMIN_USER", "CP_ADMIN_PASSWORD",
    "CP_LOCATION_ID", "CP_BOOTSTRAP_TOKEN", "CP_LICENSE_KEK_HEX", "CP_SQL_CS",
    "CP_MSSQL_CONTAINER", "CP_DB_NAME", "CP_SA_PASSWORD", "CP_PID_FILE",
    "CP_WORK_DIR", "CP_WEB_DLL",
]
with open(path, "w", encoding="utf-8") as f:
    f.write("# generated by lab-control-plane-start.sh\n")
    for k in keys:
        f.write(f"{k}={shlex.quote(os.environ.get(k, ''))}\n")
PY
