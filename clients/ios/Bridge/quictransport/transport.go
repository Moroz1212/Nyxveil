...
func (t *Transport) Dial(ctx context.Context, cfg transport.DialConfig) (transport.Conn, error) {
	...
	// iOS adapter: Frozen transport's ReadResponse doesn't observe ctx directly.
	// Close QUIC on cancellation during CONNECT, then detach on successful dial.
	stopCancel := context.AfterFunc(ctx, func() { _ = session.CloseWithError(0, "cancelled") })
	defer stopCancel()
