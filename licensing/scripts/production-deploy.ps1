...
try {
    ...
    if ($releaseVersion -cne '1.3.2') {
        throw "This wrapper requires licensing VERSION 1.3.2; found '$releaseVersion'."
    }
    ...
    if ($ReleaseZip) {
        ...
        $zipCandidates = @(
            (Join-Path $licensingRoot 'Nyxveil-ControlPlane-v1.3.2-release.zip'),
            (Join-Path (Split-Path -Parent $licensingRoot) 'Nyxveil-ControlPlane-v1.3.2-release.zip')
        )
