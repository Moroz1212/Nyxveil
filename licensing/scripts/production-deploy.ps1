...
<#
.SYNOPSIS
  Hardened emergency production deployment for Nyxveil Control Plane 1.1.1.
...
try {
    ...
    if ($ExpectedSchemaVersion -cne '2') {
        throw "Control Plane 1.1.1 requires ExpectedSchemaVersion=2."
    }
    $releaseVersion = (Get-Content -LiteralPath (Join-Path $licensingRoot 'VERSION') -Raw).Trim()
    if ($releaseVersion -cne '1.1.1') {
        throw "This wrapper requires licensing VERSION 1.1.1; found '$releaseVersion'."
    }
    ...
    if ($ReleaseZip) {
        ...
        $zipCandidates = @(
            (Join-Path $licensingRoot 'Nyxveil-ControlPlane-v1.1.1-release.zip'),
            (Join-Path (Split-Path -Parent $licensingRoot) 'Nyxveil-ControlPlane-v1.1.1-release.zip')
        )
