...
================================================================================
  ...
  seeded afterward as the operational schema marker (version 5 for CP 1.3.4).
...
BEGIN
    ALTER TABLE [CertificateRenewalOperations] DROP CONSTRAINT [CK_CertificateRenewalOperations_Status];
END;
...
BEGIN
    EXEC(N'ALTER TABLE [CertificateRenewalOperations] ADD CONSTRAINT [CK_CertificateRenewalOperations_Status] CHECK ([Status] BETWEEN 0 AND 9)');
END;
...
/* -------------------------------------------------------------------------- */
/* Operational schema version (not part of the EF model).                       */
/* Fresh bootstrap includes CertificateOperationStates (EF above) = schema 5. */
/* Existing DBs upgrade via database/migrations/005_*.sql.                    */
/* -------------------------------------------------------------------------- */
SET NOCOUNT ON;
SET XACT_ABORT ON;
GO
...
BEGIN
    ...
    (
        ...
        AppliedAt datetime2(7) NOT NULL
            CONSTRAINT DF_NyxveilSchemaVersion_AppliedAt DEFAULT (SYSUTCDATETIME()),
        AppliedBy nvarchar(128) NOT NULL
            CONSTRAINT DF_NyxveilSchemaVersion_AppliedBy DEFAULT (N'bootstrap')
    );
END
GO

-- Seed or raise to schema 5 once the EF CertificateOperationStates marker exists.
IF EXISTS (
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            SET Version = CASE WHEN Version < 5 THEN 5 ELSE Version END,
                AppliedAt = SYSUTCDATETIME(),
                AppliedBy = N'create_database'
            WHERE Id = 1 AND Version < 5;
        END
        ELSE
        BEGIN
            INSERT INTO dbo.NyxveilSchemaVersion (Id, Version, AppliedAt, AppliedBy)
            VALUES (1, 5, SYSUTCDATETIME(), N'create_database');
        END
    END
    ELSE
    BEGIN
        -- Legacy shape from migration 004 (Version/AppliedAt only).
        UPDATE dbo.NyxveilSchemaVersion
        SET Version = 5, AppliedAt = SYSUTCDATETIME()
        WHERE Version < 5;
        IF NOT EXISTS (SELECT 1 FROM dbo.NyxveilSchemaVersion WHERE Version >= 5)
            INSERT INTO dbo.NyxveilSchemaVersion (Version, AppliedAt) VALUES (5, SYSUTCDATETIME());
    END
END
GO
