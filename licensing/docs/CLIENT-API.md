...
| GET | `/catalog-keys` | **Public** (+ rate limit) | Catalog Ed25519 verification public keys only (`issuer`, `keys` kid→standard Base64, `updated_at`). Same signing ring as CatalogSigner. Never private keys. Nodes fetch without NodeAuth/license. |
